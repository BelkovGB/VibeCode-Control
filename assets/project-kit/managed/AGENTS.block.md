...
- When nodes are carried by chat sessions, the registry in `automation.sessions` says which pre-created session executes which role, and one session must never both implement and review independently. Assignments are rendered by `devflow session assign --node <id> --issue <ref>`, which prints the text and does not send it: the CLI never sees the sessions. If a session dies mid-node, restart the node whole from the last run record — that record and its head SHA are the checkpoint, and nobody continues a dead session from the middle.
