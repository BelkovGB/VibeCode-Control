...
<p align="center">
  <code>v0.1.0</code> · проекты на любом стеке · CLI: Python 3.10+ · 160 автотестов
</p>
...
- проверяет каждую ссылку на agent по реестру клиентских адаптеров и отклоняет опечатку с точным указателем, а при переносе роли на другого клиента сбрасывает выбранные model и effort вместо тихого копирования;
...
- показывает происхождение каждого гейта — требование репозитория, скилла или эскалации риска — и выдаёт минимальный набор проверок только после сверки объявленного типа изменения с фактическим диффом, не занижая при этом инварианты merge gate, артефакты review и бюджеты;
- требует по каждому узлу либо проверенный скилл, либо обоснованный `zero-skill`;
- не запускает автономную цепочку задач без явного бюджета: шаблон поставляется в режиме `manual`, а `pipeline check` отказывает при исчерпании и выдаёт готовую формулировку причины;
...
- фиксирует роли рабочих каталогов — где живёт управляющий слой, где идёт работа по Issue, где хранится временное — и ловит подмену одного другим;
- перестраивает `AGENTS.md` в компактный роутер и выносит глубину в `docs/` — отдельной командой с планом и дифом, с картой «было → стало» и с проверкой полноты: не сошлось посимвольно — план не строится, а чужой текст не сокращается ради того, чтобы поместиться;
- разводит TDD-узлы по моделям и делает спеку контрактом между ними: `tdd_red` на другой модели, чем роль, — значит тесты пишет не тот контекст, который их проектировал, и узел остаётся заблокированным, пока design не записал спеку; `spec check` проверяет структуру и двустороннее покрытие критериев кейсами, а соответствие критериев самой задаче честно оставляет ревьюеру;
- держит масштаб проекта объявленным: профиль задаёт бюджет сложности, который CLI печатает в постановке узла и не притворяется, что измерил, и дефолты гейтов, которые он действительно сравнивает; выбор и смена профиля — только с ссылкой на решение PM, а профиль-однодневка помечается флагом и ловит момент, когда проект его перерос;
- умеет исполнять узлы заранее созданными чат-сессиями: реестр «роль → сессия» типизирован, одна сессия не может и реализовывать, и независимо ревьюить, а `session assign` рендерит постановку с head SHA, ожидаемыми артефактами и командой preflight — печатает, но не отправляет, и не выдаёт живость сессии за проверенную;
...
> Автоматизация опирается на GitHub: Issues как backlog, PR и CI как внешние гейты, merge только точного проверенного SHA. Пока репозиторий не подготовлен, release-критичные гейты честно остаются `BLOCKED`. Пошаговый порядок настройки — [references/github-preparation.md](references/github-preparation.md): для каждой настройки указано, какой гейт она питает, как её проверить командой `gh` и что покажет VibeCode Control без неё.
...
Бюджет циклов принуждается CLI, а не только исполнителем. Traversal — это повторный вход: для цикла и Issue `T = максимум по узлам от (число записей узла − 1)`, поскольку узлы основного пути получают первую запись до всякой коррекции. Считаются `PASS`, `PARTIAL` и `FAIL`; `BLOCKED` и `HUMAN_NEEDED` — остановки и бюджет не тратят. `devflow operate --node <узел> --issue <ref>` показывает остаток и уходит в `BLOCKED` при исчерпании, чтобы лишний круг не начинался; `devflow run record` держит per-node страховку и отклоняет запись узла сверх `max_traversals + 1`. Продлить бюджет можно только явной ссылкой `--human-decision <ref>`: её принимают оба слоя, она попадает в запись, но бюджет не сбрасывает и `max_traversals` не поднимает — каждый следующий шаг требует её снова. Счёт ведётся по локальной истории этого checkout.
...
Текущий набор содержит 160 автоматических тестов: установка и обновление, граф, rollback, checksum drift, повреждённая конфигурация, опасные сторонние скиллы, delivery evidence, remote-gate preflight, типизированные model и effort, матрица эффективной конфигурации, миграция `config normalize` и `graph --migrate`, гейт review-артефактов, нейтральность публичного шаблона, правило декомпозиции задач, runbook подготовки GitHub, принудительный бюджет циклов, бюджет цепочки конвейера и установка пользовательского скилла.
