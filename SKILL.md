...
`AGENTS.md` is paid for on every turn of every session, so keep it a router and put depth in `docs/`. `devflow agents normalize` restructures it: a deterministic split by headings, a map of where each section went, and a refusal to write anything it cannot account for character by character. Never shorten somebody's text to make it fit — that is content loss dressed up as normalisation. Generated sections come from measured facts and carry their own markers; nested `AGENTS.md` files are listed and left alone.
