...
У этого факта есть следствие, и оно принуждается. Разные модели — значит, тесты пишет контекст, который их не проектировал; спека становится контрактом между двумя контекстами, и `operate --node tdd_red` остаётся `BLOCKED`, пока узел `design` не записал ссылку на неё для этой задачи. Совпадают модели — второго контекста нет, и требование не возникает: вечный gap для всех проектов был бы тем же классом, что F4 в #10.
