...
### Нормализация AGENTS.md

`AGENTS.md` грузится в каждую сессию каждого агента, поэтому каждый его токен оплачивается многократно. `devflow agents normalize` перестраивает файл в компактный роутер, а глубину выносит в `docs/agents/` со ссылками. Это единственная операция VibeCode Control, которая трогает файл целиком, а не свой managed-блок, — поэтому она отдельная явная команда, всегда с планом, дифом и подтверждением, в режиме `agents-normalize`, которому разрешено писать ровно три пути: `AGENTS.md`, `CLAUDE.md` и `docs/agents/**`.

**Полнота проверяется, а не обещается.** Разрез детерминированный и структурный: решают заголовки, ничего не пересказывается. После сборки каждая секция исходного файла обязана обнаружиться либо в роутере, либо в перенесённом документе — посимвольно, после нормализации пробелов. Не сошлось — план не строится, и в ошибке названа секция. Сокращение чужого текста моделью без подтверждения — потеря контента под видом нормализации, и инструмент такого не делает.

Из учёта исключены ровно две вещи, и обе по одной причине — их владелец механизм, а не автор проекта: managed-блок VibeCode Control (в карте отмечен строкой «регенерирован») и регионы генерируемых секций.

**Генерируемые секции несут собственные маркеры**, отдельные от managed-блока: `doctor` сравнивает «Сборку и проверки» с `quality.commands` измеренного baseline, а «Структуру репозитория» — с фактами inspect. Без своих маркеров drift пришлось бы искать эвристикой по тексту. Проза, занимавшая генерируемый слот, всегда переезжает в `docs/agents/`: генерация не вплавляет чужие слова в свой текст, иначе учёт полноты стал бы неразрешим. Неизмеренный baseline не порождает выдуманных команд — секция честно говорит, что проверенных команд нет, и называет команду измерения.

`CLAUDE.md` становится указателем на `AGENTS.md` со своим managed-блоком; Claude-специфичный текст владельца проходит ту же проверку полноты. Вложенные `AGENTS.md` не трогаются — их читают оба клиента, и они принадлежат своим каталогам, — но план их перечисляет: видимость без вторжения. Повторный запуск возвращает `NOT_APPLICABLE`.

### Test spec как контракт между двумя контекстами

Узел `tdd_red` может исполняться другой моделью, чем роль `implementer`: `node_overrides` задаёт model и effort по узлу. Обычный мотив — стоимость, но CLI **не ранжирует модели по цене**: прайс-оракула у него нет, а упорядочить их значило бы выдумать данные. Наблюдаем не «удешевление», а факт: модель узла пришла из node-override и отличается от модели роли.

У этого факта есть следствие, и оно принуждается. Маршрутизация — это «модель узла закреплена override'ом и **не доказано**, что она совпадает с моделью роли»; тогда `operate --node tdd_red` остаётся `BLOCKED`, пока узел `design` не записал ссылку на спеку для этой задачи.

Формула намеренно несимметрична: `routing = override explicit И НЕ (роль explicit И значения равны)`. «Тот же контекст» доказуемо ровно в одном случае — оба уровня закреплены и равны; тогда контракт не требуется, и вечного gap'а у проектов, которые не маршрутизируют, не появляется. Роль в режиме `inherited` разрешается клиентом во время запуска, сравнить нечего — и по общей дисциплине недоказуемое требует контракт, а не снимает его. Это и есть штатная однокли­ентская раскладка: роль берёт сильную модель клиента, один узел закреплён на другую, — и трактовать неразрешимое сравнение как «наверное, то же самое» значило бы снять контракт ровно там, где он и нужен.

Ссылка проверяется настолько, насколько это возможно локально. Разрешается в файл репозитория — preflight сам прогоняет проверку структуры и несёт вердикт, поэтому дыра «ссылка есть, спека — мусор» закрыта там, где это бесплатно. Не разрешается — сказано словами, что содержимое локально не проверяется; ни блока, ни фиктивного PASS, класс liveness из транспорта сессий.

Сама проверка спеки read-only и проверяет **двустороннюю** связь внутри документа: каждый кейс привязан к объявленному критерию, каждый объявленный критерий покрыт хотя бы одним кейсом, идентификаторы уникальны, тип теста — из словаря движка, у каждого кейса есть ожидаемая причина падения. Последнее поле и делает красный тест доказательством: тест, упавший не по той причине, не доказывает ничего. Соответствие идентификаторов фактическим acceptance criteria Issue машина не проверяет и об этом говорит прямо.

### Профиль масштаба и две честности

`project.scale` объявляет масштаб проекта: типизированное поле с двумя режимами — `explicit` со ссылкой на решение PM либо `undecided`, который поставляет нейтральный шаблон. Третьего не заводим: масштаб есть у каждого проекта, «неприменимо» здесь означало бы отказ отвечать на вопрос, который всё равно придётся решить.

Профиль состоит из двух частей **разной природы**, и смешивать их нельзя:

| Часть | Что это | Что делает CLI |
| --- | --- | --- |
| `gate_defaults` | дефолты параметров, которыми CLI действительно владеет | сравнивает с фактической конфигурацией и выдаёт команду применения |
| `complexity_budget` | объявленные пределы и причина | печатает в `operate` и в тексте постановки; **не измеряет** |

Микросервисы CLI считать не умеет. Держать оба вида в одном поле — значит получить фразу «инструмент проверил бюджет сложности», которая была бы ложью, тогда как «инструмент сравнил дефолты гейтов» — правда. Поэтому бюджет сложности сопровождается прямой оговоркой: он объявлен, предложение обязано на него ссылаться, а проверяет это ревьюер — узел `final_review` несёт соответствующий check.

Данные семи профилей живут в `scale-profiles.json` комплекта, устанавливаются в `.agent-flow/scale-profiles.json` и обновляются через `upgrade` — как определения этапов setup. Проект расширяет реестр блоком `project.scale_profiles`: может добавить свой профиль и переопределить канонический, ключ за ключом, как блок `clients`. Allowlist `SCALE_GATE_DEFAULT_KEYS` применяется к **итоговому** реестру, поэтому кастомный профиль не дотягивается туда, куда не дотягивается канонический.

Неминимизируемое ядро недосягаемо для профиля по построению: ключ `gate_defaults` вне allowlist — ошибка валидации, а инварианты merge gate, артефакты `evidence_contract`, бюджеты циклов и конвейера в allowlist не входят. Профиль задаёт дефолты предложений и никогда не трогает то, чему доверяют. Дефолты при этом не пишутся автоматически: `scale show` и `doctor` называют расхождение и точную команду, а запись остаётся обычным планом с дифом.

Rescale — операция, а не правка. Ссылка на решение PM обязательна и при **первом** выборе: профиль без названного решения был бы умолчанием, которого никто не утверждал. Дисциплина трёх случаев та же, что у бюджета цепочки: тот же ref и тот же профиль — тишина, тот же ref и другой профиль — отказ как тихому rescale, новый ref — запись. Сравнение идёт на записи, локальное состояние не нужно: решение живёт в самом guarded-конфиге.

«Повышение только через rescale» — флаг `promotion_watch` **в данных профиля**, а не имя `prototype` в коде. Движок читает флаг: если профиль им помечен, а конфигурация постоянно строже его дефолтов, `doctor` сообщает, что проект перерос профиль, и предлагает зафиксировать rescale. Сравнение имени было бы тем же угадыванием, что подстрока клиента до #13, и лишило бы этой защиты проект, объявивший собственный одноразовый профиль.

Подъём гейтов по risk class задачи конфигурацию не трогает и детектор не задевает: эскалация живёт в одной задаче, а `promotion_watch` ловит постоянное расхождение.
...
#### Узел выработки решения

Между `product_scope_gate` и `prepare_issue` стоит узел `design` роли `architect`: механизм **выработки** архитектурного решения, а не только его упаковки. До него `architect` умел лишь инспектировать, а артефакт architecture impact требовался на узле роли `product-lead` — то есть проектирование происходило там, где его никто не объявлял.

Выход узла — предложение: `design decision` (ADR, когда architecture impact реален), `test spec`, `issue draft`. Проверка «no unresolved product choice» остаётся: узел не принимает продуктовых решений.

Отсюда два именованных исхода, которые не схлопываются в один:

| Условие | Куда | Почему |
| --- | --- | --- |
| `design.complete` | `prepare_issue` | предложение готово, Issue упаковывается из него |
| `design.decision_required` | `human_needed` | при проектировании вскрылся продуктовый выбор вне утверждённого scope |

Возвратного ребра в `product_scope_gate` нет намеренно. «Вскрылся продуктовый выбор» — тот же класс события, на который граф уже отвечает узлом раньше (`scope.decision_required`), и отвечает терминалом `human_needed`: PM решает, прогон стартует с утверждённым scope и проходит гейт заново. Возврат ввёл бы второй плановый цикл, который пришлось бы объявлять в `allowed_cycles` и обслуживать бюджетом циклов, — вторая механика для события, у которого уже есть первая.

`prepare_issue` после этого не проектирует, а упаковывает: его `entry_condition` — `design.complete`, в `inputs` появляется `design decision`, а проверка требует, чтобы architecture impact **ссылался** на артефакт `design`, а не выносился заново. Двойное проектирование ловится проверкой, а не формулировкой.

`design` — узел стадии `planning`, поэтому запись его прогона не гейтуется именованными артефактами: как и у остальных planning-узлов сегодня, `evidence_contract` спроектирован для стадии `review`. Особого статуса у `design` нет ни в одну сторону.
...
4. `devflow graph --migrate --apply` — добавляет графу недостающие контракты артефактов review и недостающие канонические узлы;
...
Добавление недостающего канонического узла идёт той же командой и по тому же принципу «только копированием», но с более строгим предусловием: добавление узла — не вставка. Оно перевешивает ребро, которое раньше узел обходило, и правит соседний узел, который раньше делал его работу. И ребро, и сосед — территория проекта, поэтому проверяются обе оси:

- ребро, ведущее в новый узел, должно в установленном графе существовать ровно в одном экземпляре, вести в один из канонических преемников нового узла и совпадать по `max_retries` и `on_failure`;
- затрагиваемый сосед должен совпадать с каноном во всех полях, кроме мигрируемых, иметь `entry_condition`, равный условию перевешиваемого ребра, а его `inputs` и `checks` — быть каноническими списками ровно без добавляемых элементов.

Дособственные элементы соседа означают, что проект принял там решение; миграция его не переписывает. Любое расхождение по любой оси перечисляется в `requires_explicit_decision` с указанием, что именно разошлось, и узел не добавляется — а не добавляется частично. Проверять только ребро было бы хуже, чем не проверять ничего: узел появился бы, а `entry_condition` соседа остался бы прежним — валидатор это пропускает, потому что не сверяет условие узла с условиями рёбер, и граф стал бы семантически лживым.
