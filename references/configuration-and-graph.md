...
The same command also adds canonical nodes the installed graph does not have yet, and reports them under `added_nodes`. Adding a node is not an insertion: it rewires the edge that used to bypass the node and edits the neighbour that used to carry its work, so both are compared with canon before anything is written. The edge into the new node must exist exactly once in the installed graph, lead to one of the new node's canonical successors, and match on `max_retries` and `on_failure`. The affected neighbour must match canon on every field the migration does not touch, carry the rewired edge's condition as its `entry_condition`, and hold `inputs` and `checks` that are the canonical lists minus exactly the entries being added. Anything else — a changed retry limit, a check the project wrote itself — means the project made a decision there, and the migration refuses to overwrite it: the node is listed under `requires_explicit_decision` with what diverged, and nothing is written. Checking only the edge would be worse than checking nothing, because the node would appear while the neighbour kept its old `entry_condition`; the validator does not compare a node's entry condition with its incoming edges, so the graph would pass validation while lying about itself.
...
Keys outside the allowlist are a validation error, applied to the merged registry, so no custom profile reaches further than a canonical one and the non-minimizable core stays out of reach by construction. Defaults are never written automatically: `devflow scale show` and `devflow doctor` report each gate that is weaker than the profile together with the exact `config set` command, and the write stays an ordinary plan with a diff. When a profile carries `promotion_watch` and the configuration is permanently stricter than its defaults, the report says the project has outgrown the profile and asks for a rescale — the engine reads the flag and never matches the profile name.

`automation.sessions` records which pre-created chat session executes which role: `{"mode": "explicit", "client": ..., "roles": {"<role>": {"mode": "explicit", "session": "<name>"}}}`, or `not-applicable` when the project runs no sessions, or `undecided` until the owner chooses. The client must be an adapter that declares `session_messaging`, and every role name must exist in `config.roles`. A binding is checked against the role's own agent one floor earlier than the run record would catch it: a role whose agent belongs to another client, or whose agent executes no model, cannot be bound to a session, and validation names the exact pointer. A role whose agent is still `unresolved` may be bound — the pending executor decision already lives on the `roles` stage and is not turned into a second permanent gap. A role whose agent is `human`, `script`, or `deterministic` is not asked about a session at all: an agent that executes no model receives no assignment through this transport by definition, so `not-applicable` is derived rather than written into the configuration, and giving that role an executing agent brings the question back. The shipped kit declares `undecided` and names no session, so the `automation` setup stage asks. Validation resolves node → session pairs and rejects a registry where one session executes both an `implementation` node and a `review` node of a different role — the independent review has to be independent, while `implementer_review` stays legal because it belongs to the implementer's own role. The refusal happens at write time: `devflow config set` compares the graph errors before and after the change and rejects exactly what the change introduces.
