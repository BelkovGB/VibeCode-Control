...
`automation.sessions` records which pre-created chat session executes which role: `{"mode": "explicit", "client": ..., "roles": {"<role>": {"mode": "explicit", "session": "<name>"}}}`, or `not-applicable` when the project runs no sessions, or `undecided` until the owner chooses. The client must be an adapter that declares `session_messaging`, and every role name must exist in `config.roles`. The shipped kit declares `undecided` and names no session, so the `automation` setup stage asks. Validation resolves node → session pairs and rejects a registry where one session executes both an `implementation` node and a `review` node of a different role — the independent review has to be independent, while `implementer_review` stays legal because it belongs to the implementer's own role. The refusal happens at write time: `devflow config set` compares the graph errors before and after the change and rejects exactly what the change introduces.
