...
The design node also produces the test specification, and its format is a contract rather than a habit. Every criterion carries a stable identifier (`FR-###`, `SC-###`, `US#`, `AC#`) and every case names the criterion it covers, its type from `unit | integration | contract | e2e`, its Given/When/Then triple, and the reason the test is expected to fail before the change exists. That last field is what makes a red test evidence: a test failing for the wrong reason proves nothing, and `test fails for intended reason` has something concrete to compare against. `devflow spec check <path>` verifies the structure and the coverage in both directions — every case bound to a declared criterion, every declared criterion covered by a case — and says plainly that matching those identifiers to the real Issue is the reviewer's job.

The node writing the tests does not invent conditions. An ambiguous or incomplete specification is `BLOCKED` and goes back to design; a plausible guess that fills the gap is the failure this rule exists to prevent.

Granularity below an Issue does not become a tracked file. Steps, checklists, and working notes inside one Issue live in the run records, the PR, and the working conversation — not in a second backlog that has to be kept in sync with the first. An Issue is the smallest unit the process tracks; anything smaller is how the work is done, not what is owed.
